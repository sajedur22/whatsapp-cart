...
export default function CartModal({ cart, total, form, setForm, updateQty, onClose, onSend,clearCart }) {
      ...
      <div className="fixed inset-0 font-mono flex items-center justify-center bg-black/90 z-50">
        <div className="bg-white rounded-lg w-full max-w-lg p-6 shadow-lg relative">
                    ...
                    <li key={item.id}>
                      <div className="flex justify-between items-center gap-2">
                        ...
                        <input
                            ...
                            className="w-12 border-b-2 border-emerald-700 outline-none"
                        />
          ...
          <div className="mt-4 flex flex-row gap-2">
            <button
                onClick={onSend}
                className="flex-1 cursor-pointer bg-emerald-700 text-white px-4 py-2 rounded hover:scale-105 duration-300"
            >
              Send Order via WhatsApp
            </button>

            <button
                onClick={() => clearCart()} // শুধু ওই প্রোডাক্ট রিমুভ হবে
                className="flex-1 cursor-pointer bg-emerald-700 text-white px-4 py-2 rounded hover:scale-105 duration-300"
            >
              Remove Item
            </button>
